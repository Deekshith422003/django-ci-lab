...
class DogAPITestCase(TestCase):
    def setUp(self):
        ...
        self.dog_data = {'name': 'Aachi', 'breed': 'Labrador', 'age': 5}
        self.response = self.client.post('/dogs/', self.dog_data, format='json')

    def test_create_dog(self):
        self.assertEqual(self.response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Dog.objects.count(), 1)
        self.assertEqual(Dog.objects.get().name, 'Aachi')

    def test_get_dog(self):
        dog = Dog.objects.create(name='Tommy', breed='German Sheperd', age=3)
        response = self.client.get(f'/dogs/{dog.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['name'], 'Tommy')

    def test_update_dog(self):
        dog = Dog.objects.get()
        new_data = {'name': 'Aachi', 'breed': 'Labrador', 'age': 6}
